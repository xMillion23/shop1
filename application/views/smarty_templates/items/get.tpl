        ...
        <blockquote class="blockquote text-center">
            ...
            <div class="card bg-light card-body mb-3">
                {if $item.main_image.hash !== 'default'}
                    ...
                    {capture name="t_item_img"}<img class="img-fluid" src='{$smarty.capture.t_img}' title='{$item.name|escape:"html":"UTF-8"}'>{/capture}
                    {capture name="t_item_url"}item/{$item.hash}{/capture}
                    {url type="anchor" url=$smarty.capture.t_item_url text=$smarty.capture.t_item_img attr='title="{$item.name|escape:"html":"UTF-8"}"'}
                {/if}<hr/>
                    <div class="caption-full">       
                    <h4>{url type="anchor" url=$smarty.capture.t_item_url text=$item.name|escape:"html":"UTF-8" attr="title='{$item.name|escape:"html":"UTF-8"}'"}<br/><br/>{$item.price_f|escape:"none"}</h4><hr/>
                    <div class="form">
                        {if $current_user.logged_in == TRUE}
                            {if $current_user.user_hash != $item.vendor.user_hash}{url type="anchor" url=$smarty.capture.t_message_vendor_url text="Message" attr='class="btn btn-warning"'}{/if}
                    ...
                    <p>{nl2br($item.description|escape:"html":"UTF-8")}</p><hr/>
                ...
                <div class="ratings">
                    <p class="form">
                        ...
                        {rating rating=$item.average_rating}</p>
                </div><hr/>
            ...
            <div class="card bg-light card-body mb-3">
                ...
                <div class="col-row">
                    <div class="col-row">
                        ...
                        <span class="col-row">{$review.time_f}</span>
